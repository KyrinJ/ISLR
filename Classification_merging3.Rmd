...
lda.fit=lda(stage~miRNA294+miRNA215,data=train.tree)
...
for (i in 1:641){
  if (i==294){
    ...
    set.seed(114)
    lda.fit=lda(stage~miRNA294+train.tree[,i+1],data=train.tree)
    lda.pred = predict(lda.fit, test.tree)
    roc=multiclass.roc(ordered(test.tree$stage),ordered(lda.pred))
